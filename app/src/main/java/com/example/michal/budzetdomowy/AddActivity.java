...
public class AddActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((ListView)findViewById(R.id.lista4)).setAdapter(
                scAdapter);
        getLoaderManager().initLoader(1, null, this);

        System.out.println("saldo: " + saldoManager.saldo);
        try {
            saldoManager.aktualizacja_salda2();
        }
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        return new CMaterialLoaderAdd(this, saldoManager.dbMaterial4);
    }
        ...
        switch(view.getId()){
            case R.id.button:
                try {
                    ...
                    saldoManager.dbMaterial4.addMaterial(
            ...
            case R.id.button2:
                try {
                    ...
                    String deletingValue = saldoManager.dbMaterial4.getSingular_Value_InTransaction("SELECT typ FROM TYP WHERE _id = '" + fieldValue3 + "'");

                    saldoManager.saldo += Integer.parseInt(deletingValue);

                    System.out.println(saldoManager.dbMaterial4.getSingular_Value_InTransaction("SELECT typ FROM TYP WHERE _id = '"+fieldValue3+"'"));
                    //System.out.println(dbMaterial3.getValue(Integer.parseInt(fieldValue)));
                    System.out.println(saldoManager.saldo);

                    saldoManager.dbMaterial4.deleteMaterial(Integer.parseInt(fieldValue3));
                }
                catch(NumberFormatException ex){
                    System.out.println("zla wartosc");
                }
